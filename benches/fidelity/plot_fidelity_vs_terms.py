...
with (directory / filename + ".json").open() as file:
...
for method, values in results.items():
    ...
    terms = [int(label.split()[0]) for label in labels]
    data = np.array([values[label] for label in labels], dtype=float)
    mean = data.mean(axis=1)
    minimum = data.min(axis=1)
    maximum = data.max(axis=1)

    ax.plot(terms, mean, marker="o", color=color, label=method)
    ax.fill_between(terms, minimum, maximum, color=color, alpha=0.2)
